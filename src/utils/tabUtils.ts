...
export const resetTab = (id: string) => {
  ...
  if (tab) {
    ...
    tab.style.height = 200 + "px";
    tab.style.borderRadius = 100 + "px";
    tab.style.transform = `translate(${id === "left" ? "-" : ""}100px, -100px)`;
  }
};
...
export const moveTab = (event: MouseEvent, id: string) => {
  ...
  if (tab) {
    ...
    if (distanceX < 400 && distanceY < 400) {
      ...
      const newWidth = Math.min(400, Math.max(200, 200 + 100 * powX));
      const newHeight = Math.min(400, Math.max(200, 200 + 100 * powY));
